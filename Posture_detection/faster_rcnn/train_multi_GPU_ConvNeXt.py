# -*- codeing = utf-8 -*-
# @Time: 2022/2/12 13:24
# @Author:xpwang
# @File:train_multi_GPU_ConvNeXt.py
# @Software:PyCharm
import time
import os
import datetime
...
from backbone import ConvNeXt_fpn
...
def create_model(num_classes):
    ...
    #                                  trainable_layers=3)
    # backbone = efficv2_fpn.mobilenet_fpn_backbone(pretrain_path="./backbone/pre_efficientnetv2-m.pth")
    backbone = ConvNeXt_fpn.ConvNeXt_fpn_backbone(pretrain_path="./backbone/convnext_tiny_1k_224_ema.pth")
...
def main(args):
    ...
    # 用来保存coco_info的文件
    results_file = "C_results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.rank in [-1, 0]:
            ...
            with open(results_file, "a") as f:
                ...
                f.write(txt + "\n")
        if args.output_dir:
            if coco_map > best_coco_map:
                best_coco_map = coco_map
                # 只在主节点上执行保存权重操作
                save_on_master({
...
if __name__ == "__main__":
    ...
    # TODO 下面这个也要改
    parser.add_argument('--lr-steps', default=[70, 120], nargs='+', type=int, help='decrease lr every step-size epochs')
    # 针对torch.optim.lr_scheduler.MultiStepLR的参数
    parser.add_argument('--lr-gamma', default=0.1, type=float, help='decrease lr by a factor of lr-gamma')
    # 训练过程打印信息的频率
    parser.add_argument('--print-freq', default=20, type=int, help='print frequency')
    # 文件保存地址
    parser.add_argument('--output-dir', default='./multi_train_C', help='path where to save')
    # 基于上次的训练结果接着训练
