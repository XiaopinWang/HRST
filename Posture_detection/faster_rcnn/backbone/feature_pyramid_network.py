...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(self, in_channels_list, out_channels, extra_blocks=None):
        ...
        for in_channels in in_channels_list:
            if in_channels == 0:
                continue
            inner_block_module = nn.Conv2d(in_channels, out_channels, 1)  # 1*1的卷积核用来调整不同层上的channel
    ...
    def forward(self, x):
        ...
        last_inner = self.get_result_from_inner_blocks(x[-1], -1)  # 20*20*2048 --> 20*20*256
        # result中保存着每个预测特征层
        results = []
