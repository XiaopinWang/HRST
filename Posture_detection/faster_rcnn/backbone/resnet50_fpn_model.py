...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True, norm_layer=None):
        ...
        self.conv1 = nn.Conv2d(3, self.in_channel, kernel_size=7, stride=2,
                               padding=3, bias=False)  # down2
        self.bn1 = norm_layer(self.in_channel)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # down2
        self.layer1 = self._make_layer(block, 64, blocks_num[0])  # 64-->256
        self.layer2 = self._make_layer(block, 128, blocks_num[1], stride=2)  # 256-->512  down2
        self.layer3 = self._make_layer(block, 256, blocks_num[2], stride=2)  # 512-->1024 down2
        self.layer4 = self._make_layer(block, 512, blocks_num[3], stride=2)  # 1024-->2048 down2
        if self.include_top:
