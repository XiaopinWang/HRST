...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        # original_image_sizes: List[Tuple[int, int]] = []  # 用于存放原始的图像尺寸 c,h,w
        for img in images:
            val = img.shape[-2:]
            assert len(val) == 2  # 防止输入的是个一维向量
            original_image_sizes.append((val[0], val[1]))  # 原始图像尺寸
        # original_image_sizes = [img.shape[-2:] for img in images]

        images, targets = self.transform(images, targets)  # 对图像进行预处理(Normalize, Resize)
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 # transform parameter  num_classes需要加上背景20+1
                 min_size=800, max_size=1280,      # 预处理resize时限制的最小尺寸与最大尺寸
