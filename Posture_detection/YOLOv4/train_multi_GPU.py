...
def main(opt, hyp):
    ...
    wdir = "YOLOv4_24hao" + os.sep  # weights dir
    best = wdir + "best.pt"
    results_file = "YOLOv4_24hao_results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    accumulate = max(round(16 / (opt.world_size * opt.batch_size)), 1)
    ...
    #    try:
    #        #ckpt = {k: v for k, v in ckpt["model"].items() if model.state_dict()[k].numel() == v.numel()}
    #        # numel() Returns the total number of elements in the input tensor.
    #        #model.load_state_dict(ckpt["model"], strict=False)  # ckpt["down1.conv1.conv.0.weight"]
    ...
    if weights.endswith(".pt") or weights.endswith(".pth"):
        ...
        # load model
        try:
            ckpt["model"] = {k: v for k, v in new_state_dict.items()
            ...
            if opt.rank in [-1, 0]:
                ...
                if opt.savebest is False:
                    ...
                    with open(results_file, 'r') as f:
                        ...
                        torch.save(save_files, "./weights/yolov4spp-{}.pt".format(epoch))
                else:
                    ...
                    if best_map == coco_mAP:
                        with open(results_file, 'r') as f:
                            ...
                            if opt.amp:
                                save_files["scaler"] = scaler.state_dict()
                            #torch.save(save_files, best)
                            torch.save(save_files, "./weights_YOLOv4/final_yolov4spp-{}.pt".format(epoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', type=int, default=832, help='test size')
   ...
   #                     help='initial weights path')
    parser.add_argument('--weights', type=str, default='weights/yolo4_weights.pth',
                        help='initial weights path')
