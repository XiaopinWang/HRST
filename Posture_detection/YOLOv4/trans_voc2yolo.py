...
# voc数据集根目录以及版本
voc_root = "D:\pig_gesture\pig_voc\VOCdevkit\VOC2012"
voc_version = "VOC2012"
...
test_txt = "test.txt"
# 转换后的文件保存目录
save_file_root = "D:\pig_gesture\pig_voc"

# label标签对应json文件
label_json_path = './pig_data/pascal_voc_classes.json'
...
test_txt_path = os.path.join(voc_root,  "ImageSets", "Main", test_txt)
# 检查文件/文件夹都是否存在
...
def translate_info(file_names: list, save_root: str, class_dict: dict, train_val='train'):
    ...
    for file in tqdm(file_names, desc="translate {} file...".format(train_val)):
        ...
        if os.path.exists(path_copy_to) is False:
            ...
            # 将src文件内容复制到dst的文件上。如果dst文件不存在，程序自动创建，如果已存在，则覆盖
            # copy 如果dst为目录，则表示复制文件到该目录下;反之如果是文件，则表示创建指定文件再复制src的数据到该文件上。
            # copy() 仅复制原文件的数据和权限，其它元数据不会保留。需要保留更多的元数据可使用copy2()。
