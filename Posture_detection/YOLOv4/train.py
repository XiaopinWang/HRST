...
def train(hyp):
    ...
    best = wdir + "best.pt"  # TODO 查看是否保存模型最好的权重
    ...
    # fmod()用来对浮点数进行取模(求余)  y%x运算总是将y/x的值向下取整，而math.fmod(y,x)是将y/x的值向0取整
    ...
    hyp["obj"] *= imgsz_test / 320  # 对损失的增益进行调整

    # Remove previous results
    for f in glob.glob(results_file):
        os.remove(f)
        # os.remove() 方法用于删除指定路径的文件
    # 先训练3个预测器随后训练除Dark53之后的所有权重能够达到一个更好的效果
    # Initialize model
    ...
    # todo 先进行3个预测器的训练，后进行出特征提取网络的其他权重能够达到一个较好的结果
    else:
    ...
    optimizer = optim.SGD(pg, lr=0.01, momentum=0.9,
    ...
    if weights.endswith(".pt") or weights.endswith(".pth") or weights.endswith(".weights"):
        ckpt = torch.load(weights, map_location=device)
        try:
            # ckpt = {k: v for k, v in ckpt.items() if model.state_dict()[k].numel() == v.numel()}
            # # numel() Returns the total number of elements in the input tensor.
            # model.load_state_dict(ckpt, strict=False)  # ckpt["down1.conv1.conv.0.weight"]
    ...
    #     ckpt = torch.load(weights, map_location=device)
    #     # load model
    #     for idx, b in enumerate(ckpt.values()):
    #         new_state_dict[str_ex[idx]] = b
    #     try:
    #         ckpt = {k: v for k, v in new_state_dict.items() if model.state_dict()[k].numel() == v.numel()}
    #         # numel() Returns the total number of elements in the input tensor.
    #         model.load_state_dict(ckpt, strict=False)  # ckpt["down1.conv1.conv.0.weight"]
    #         # ckpt["model"] = {k: v for k, v in ckpt["model"].items() if model.state_dict()[k].numel() == v.numel()}
    #         # # numel() Returns the total number of elements in the input tensor.
    ...
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[70, 120], gamma=0.1)
    ...
    # 遍历验证集将他的标签全部读取一遍，方便使用coco计算map
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', type=int, default=4)  # TODO 这个以后需要修改
    parser.add_argument('--cfg', type=str, default='cfg/my_yolov4.cfg', help="*.cfg path")
    parser.add_argument('--data', type=str, default='data/my_data.data', help='*.data path')
    parser.add_argument('--hyp', type=str, default='cfg/hyp.yaml', help='hyperparameters path')
    parser.add_argument('--multi-scale', type=bool, default=True,
                        help='adjust (67%% - 150%%) img_size every 10 batches')
    parser.add_argument('--img-size', type=int, default=832, help='test size')  # 1280
    # 指定网络输入图片的大小，测试过程中输入图片大小，训练过程中不会控制输入图片大小
    parser.add_argument('--rect', action='store_true', help='rectangular training')
    parser.add_argument('--savebest', type=bool, default=True, help='only save best checkpoint')  # todo 保存精度最高的模型权重
    parser.add_argument('--notest', action='store_true', help='only test final epoch')  # 若为True在模型最后一个epoch进行验证
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training')  # yolo4_weights.pth
    parser.add_argument('--weights', type=str, default='weights/final_yolov4spp-185.pt',  # yolov3-spp-ultralytics-512.pt
                        help='initial weights path')  # 如果想继续训练，将模型权重保存为最后一次的权重就行
    ...
    # 若为True只会训练yolo predict 若为false除了主干外都可以进行训练
