...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        self.deconv_with_bias = extra.DECONV_WITH_BIAS  # fasle
    ...
    def init_weights(self, pretrained=''):
        if os.path.isfile(pretrained):
            ...
            #checkpoint = torch.load(pretrained,map_location=torch.device('cpu'))  # TODO 这里面改了
            checkpoint = torch.load(pretrained)
            checkpoint["final_layer.weight"] = checkpoint["final_layer.weight"][:10, :, :, :]
            checkpoint["final_layer.bias"] = checkpoint["final_layer.bias"][:10,]
