...
class eca_block(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.size()
        avg = self.avg_pool(x).view([b, 1, c])  # batch, 每一个step的特征长度, 每一个时序
        out = self.conv(avg)
...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        num_channels = self.stage3_cfg['NUM_CHANNELS']  # 48,96,192
        ...
        #        stride=1,
        #        padding=0),
        #    nn.BatchNorm2d(720, momentum=BN_MOMENTUM),
    ...
    def forward(self, x):
        ...
        for i in range(self.stage2_cfg['NUM_BRANCHES']):
            if self.transition1[i] is not None:
                ...
                x_list.append(x)
        y_list = self.stage2(x_list)  # 融合之后的
...
#
    ...
    def init_weights(self, pretrained=''):
        ...
        if os.path.isfile(pretrained):
            pretrained_state_dict = torch.load(pretrained, map_location="cpu")  # TODO 这部分改变了
