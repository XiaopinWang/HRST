...
#
#     def __len__(self):
#         return self.enc_inputs.shape[0]  # [2, 5]
#
...
#         self.register_buffer('pe', pe)  # 定义一个缓冲区，参数不参与更新
#
#     def forward(self, x):
#         """
...
#         dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(dec_inputs, enc_inputs, enc_outputs)  # TODO 为什么会有enc_inputs
...
test_file_path = r"D:\crop_picture\backup\total_data\chage_try\delete_pig\test.txt"# txt
train_list_img = []  # img_path
test_img_list = []
train_file_path_img = r"D:\crop_picture\backup\total_data\chage_try\delete_pig\test_img"  # img
test_img = r"D:\crop_picture\backup\total_data\chage_try\delete_pig\test_img"
with open(test_file_path)as f3:
    for v in f3.readlines():
        train_list.append(v.strip())
for k in os.listdir(train_file_path_img):
    k = k.strip()
    train_list_img.append(k.split(".")[0])
# # for w in os.listdir(test_img):
# #     w = w.strip()
# #     test_img_list.append(w.split(".")[0])
count = 0
for q in train_list_img:
    if q not in train_list:
        count += 1
        print(q)
# print(count)
