...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
                 ...
                 img_size=416,
                 batch_size=16,
                 augment=False,  # 训练集设置为True(augment_hsv)，验证集设置为False  是否开启颜色对比度的增强
                 hyp=None,  # 超参数字典，其中包含图像增强会使用到的超参数
                 rect=False,  # 是否使用rectangular training  训练集设置为false验证时设置为true
        ...
        if self.rect:
            ...
            for i in range(nb):
                ...
                if maxi < 1:
                    shapes[i] = [maxi, 1]  # 设置为指定尺度的比例
                # 如果高/宽大于1(w < h)，将h设置为img_size
                elif mini > 1:
                    shapes[i] = [1, 1 / mini]
            # 计算每个batch输入网络的shape值(向上设置为32的整数倍) [0.75, 1]
        ...
        # 缺少标签的数据，找到了多少数据，有多少标签是空的，标签中多有少是重复的
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                if extract_bounding_boxes:
                    # 如果设定为True，会将每个目标裁剪出来，按照相应类别进行存储
            ...
            # 处理进度条只在第一个进程中显示  每遍历一次就更新一次提示信息
            if rank in [-1, 0]:
                # 更新进度条描述信息
        ...
        if cache_images:  # if training
            ...
            for i in pbar:  # max 10k images
                ...
                gb += self.imgs[i].nbytes  # 用于记录缓存图像占用RAM大小  返回数组元素消耗的总字节数
                if rank in [-1, 0]:
        ...
        if detect_corrupted_images:
            ...
            for file in tqdm(self.img_files, desc="Detecting corrupted images"):
                try:
                    _ = io.imread(file)  # 如果读取图片出现错误，图片有破损
                except Exception as e:
    ...
    def __getitem__(self, index):
        ...
        img = np.ascontiguousarray(img)  # 转换成内存连续的数据
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        img4[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]  w, h
    ...
    img4, labels4 = random_affine(img4, labels4,
                                  ...
                                  border=-s // 2)  # border to remove  s为期待输出图像的大小
