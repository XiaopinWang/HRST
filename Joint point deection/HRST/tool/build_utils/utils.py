...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    ...
    if ratio_pad is None:  # calculate from img0_shape  # torch.Size([512, 832]) img1_shape
        gain = max(img1_shape) / max(img0_shape)  # gain  = old / new  # 0.325 # (1440, 2560, 3) img0_shape
...
#     iou = inter / (area_b1+area_b2-inter)
#     return iou
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    ...
    if x1y1x2y2:  # x1, y1, x2, y2 = box1
        ...
        b1_x1, b1_x2 = box1[0] - box1[2] / 2, box1[0] + box1[2] / 2  # 左上角x, 右下角x
        b1_y1, b1_y2 = box1[1] - box1[3] / 2, box1[1] + box1[3] / 2  # 左上角y, 右下角y
...
def wh_iou(wh1, wh2):
    ...
    wh1 = wh1[:, None]  # [N,1,2]  # TODO 怎么增加新的维度
    wh2 = wh2[None]  # [1,M,2]
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    # class label smoothing ht  tps://arxiv.org/pdf/1902.04103.pdf eqn 3
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            # giou = bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, GIoU=True)  # giou(prediction, target)  预测值和真实gt的GIOU
            ciou = bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, CIoU=True)
            lbox += (1.0 - ciou).mean()  # giou loss
            # Obj
...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate(model.yolo_layers):  # [89, 101, 113]
        ...
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain  [1,1,w,h,w,h]
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6,
    ...
    # torch.Size([1, 26208, 7]) prediction
    t = time.time()
...
# def non_max_suppression(prediction, conf_thres=0.1, nms_thres=0.6):
#     """
...
#         pred = torch.cat((pred[:, :4], class_conf.unsqueeze(1), class_pred), 1)
#
